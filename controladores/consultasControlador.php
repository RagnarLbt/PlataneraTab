...
//Consultas extras 
        ...
        $op=$_POST['op'];
            ...
            if($op==1){ //entonces es por embarque
                ...
                $idEmb2 = $_POST['idEmb2'];
                    ...
                    if ($idPro[0]==9999 && $idEmb!=0 && $idEmb2==0){
                        $query=mainModel::ejecutar_consulta_simple("SELECT round(SUM(fruta.peso_kg),2) as e from fruta 
                            where fruta.id_embarque=$idEmb GROUP BY fruta.id_embarque, fruta.id_productores");
                        return $query->fetchAll(PDO::FETCH_ASSOC);
                    }
                    //6.-Todos los productores en un rango de embarque
                        ...
                        $query=mainModel::ejecutar_consulta_simple("SELECT round(SUM(fruta.peso_kg),2) as e from fruta
                        where fruta.id_productores=$idPro[0] and fruta.id_embarque=$idEmb");
                        return $query->fetchAll(PDO::FETCH_ASSOC);
                    }
                    ...
                    elseif(count($idPro)>1 && $idEmb!=0 && $idEmb2!=0){
                        ...
                        $Prod=implode(",", $array);
                        $query=mainModel::ejecutar_consulta_simple("SELECT round(sum(fruta.peso_kg),2) as e from fruta
                            where fruta.id_productores in ($Prod) and fruta.id_embarque between $idEmb and $idEmb2  GROUP BY  fruta.id_productores 
                            ORDER BY fruta.id_productores ASC");
                        return $query->fetchAll(PDO::FETCH_ASSOC);
                    }
                ...
                elseif(count($idPro)>1 && $fecha1!="" && $fecha2==""){
                    ...
                    $query=mainModel::ejecutar_consulta_simple("SELECT round(SUM(fruta.peso_kg),2) as e from fruta 
                    INNER JOIN productor_fruta on fruta.id= productor_fruta.id_fruta
                    where  productor_fruta.fecha_compra='$fecha1' and fruta.id_productores in ($Pel)
                    GROUP BY  fruta.id_productores order by fruta.id_productores asc");
                    return $query->fetchAll(PDO::FETCH_ASSOC);
                }
                ...
                elseif(count($idPro)>1 && $fecha1!="" && $fecha2!=""){
                    ...
                    $query=mainModel::ejecutar_consulta_simple("SELECT round(SUM(fruta.peso_kg),2) as e from fruta 
                    INNER JOIN productor_fruta on fruta.id= productor_fruta.id_fruta
                    where  productor_fruta.fecha_compra between '$fecha1' and '$fecha2' and fruta.id_productores in ($Pel)
                    GROUP BY  fruta.id_productores
                    ORDER BY  fruta.id_productores asc");
                    return $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function rendimientoEmbarqueX_controlador(){
        ...
        if ($op==1){
            ...
            //5.-Todos los productores en un embarque
            if ($idPro[0]==9999 && $idEmb!=0 && $idEmb2==0){
                $query=mainModel::ejecutar_consulta_simple("SELECT concat(productores.nombre,' ', productores.Ap_p) as e from fruta
				INNER JOIN productores on fruta.id_productores=productores.id
                where fruta.id_embarque=$idEmb GROUP BY fruta.id_embarque, fruta.id_productores");
                return $query->fetchAll(PDO::FETCH_ASSOC);
            }
            ...
            //Un productor en un embarque
                ...
                $query=mainModel::ejecutar_consulta_simple("SELECT concat(productores.nombre,' ', productores.Ap_p) as e from fruta
				INNER JOIN productores on fruta.id_productores=productores.id where fruta.id_productores=$idPro[0] and fruta.id_embarque=$idEmb");
                return $query->fetchAll(PDO::FETCH_ASSOC);
            }
            ...
            //3.-Un productor en un rango de embarques
                ...
                $query=mainModel::ejecutar_consulta_simple("SELECT concat(productores.nombre,' ', productores.Ap_p) as e from fruta
				INNER JOIN productores on fruta.id_productores=productores.id
                where fruta.id_productores=$idPro[0] and fruta.id_embarque BETWEEN $idEmb and $idEmb2 GROUP BY fruta.id_productores");
                return $query->fetchAll(PDO::FETCH_ASSOC);
            }
