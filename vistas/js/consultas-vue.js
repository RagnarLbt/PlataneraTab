...
methods:{
  ...
  //Pago peladores
    ...
    btnConsultaP4(idPro, idEmb, idEmb2){
      if(idEmb!=0 && idEmb2 != 0 && (idPro.length>1 || idPro[0]=='9999') ){
        ...
        Swal.fire({
          icon:'error',
          title: 'No se econtraron embarques o productores',
          text: 'Seleccione un rango de embaques y multiples productores',
          backdrop: false
        });
      }
    },
    btnConsultaP4Fecha(idPro, fech1, fech2){
      if(fech1!='' && fech2!='' && fech1<fech2 && (idPro.length>1 || idPro[0]=='9999') ){
        ...
        Swal.fire({
          icon:'error',
          title: 'No se seleccionó fechas o productores',
          text: 'Seleccione un rango de fechas y multiples productores',
          backdrop: false
        });
      }
    },
  ...
  //Nuevas consultas
    ...
    btnGraficaP4(){
       ...
       this.graficar(this.dataGaficaMultiLinea, this.p6, 'Peso fruta', 'graficaP4', '');
    },
    btnGraficaP4fecha(){
      //this.cargarGraficaL(this.p5F, this.p6F, 'Peso fruta', 'graficaP4','url5');
      this.graficar(this.dataGaficaMultiLinea, this.p6F, 'Peso fruta', 'graficaP4', '');
    },
  //Grafica bolsas peladores
  ...
  //Grafica Pago Peladores
    ...
    graficar(datosY, datosX, texto , id, titulo){
      if(window.graficaMult){
        window.graficaMult.clear();
        window.graficaMult.destroy();
      }
      var ctx = document.getElementById(id).getContext('2d');

      window.graficaMult = new Chart(ctx, {
          type: 'line',
          data: {
              labels: datosX,
              datasets: datosY,
  ...
  //========================
    listarProd4(idPro,idEmb, idEmb2){
      this.totalfech=0;
  ...
  //Grafica datos y (peso)
    listarDatosYProd4(idPro, idEmb, idEmb2){
      ...
      axios.post(url_consultas,{option:21, op:1, idPro:idPro, idEmb:idEmb, idEmb2:idEmb2}).then(response=>{
        ...
        //Guardar los id de productores repedidos tal y como viene de la consulta SQL
        this.listaYProd4.forEach( item => this.labels.push(item.id));
        //Recorrer todos los productores
        for (var i = 0;  i < this.labels.length; i++) {
          this.fruta_kg=[];
          //Comparara si los productores son diferentes al anterior iteración
          if(i==0 || this.labels[i]!=this.labels[i-1]){
            
            for (var x = 0; x < this.p6.length; x++) {
              //Recorrer el array de datos de fruta para cada uno de los prodcutores...
              for (var j = 0; j < this.listaYProd4.length; j++) {

                //Comparar que los productores sean iguales...
                if(parseInt(this.p6[x]) == parseInt(this.listaYProd4[j].id_embarque) && this.labels[i] == this.listaYProd4[j].id){
                  this.fruta_kg.push(this.listaYProd4[j].peso_kg);
                  break;
                }else if(parseInt(this.p6[x]) < parseInt(this.listaYProd4[j].id_embarque) && this.labels[i] == this.listaYProd4[j].id){
                  this.fruta_kg.push(0);
                  break;
                }
                //Generar los array 
                auxDta={label: this.labels[i], borderColor: 
                  'rgb('+Math.floor(Math.random()*256)+
                  ','+Math.floor(Math.random()*256)+','
                  +Math.floor(Math.random()*256)+')', data: this.fruta_kg};
              }
            }
            
            this.dataGaficaMultiLinea.push(auxDta);
          }
        }
        //console.log(this.dataGaficaMultiLinea);
      });
    },
    ...
    listarDatosYProd4Fecha(idPro, fech1, fech2){
      ...
      axios.post(url_consultas,{option:21,op:2, idPro:idPro, fech1:fech1, fech2:fech2}).then(response=>{
        ...
        //Guardar los id de productores repedidos tal y como viene de la consulta SQL
        this.listaYProd4F.forEach( item => this.labels.push(item.id));
        ...
        for (var i = 0;  i < this.labels.length; i++) {
          ...
          if(i==0 || this.labels[i]!=this.labels[i-1]){
            ...
            for (var x = 0; x < this.p6F.length; x++) {

              //Recorrer el array de datos de fruta para cada uno de los prodcutores...
              for (var j = 0; j < this.listaYProd4F.length; j++) {
                //Comparar que los productores sean iguales...
                if(parseInt(this.p6F[x])== parseInt(this.listaYProd4F[j].id_embarque) && this.labels[i] == this.listaYProd4F[j].id){
                  this.fruta_kg.push(this.listaYProd4F[j].peso_kg);
                  break;
                }else if(parseInt(this.p6F[x]) < parseInt(this.listaYProd4F[j].id_embarque) && this.labels[i] == this.listaYProd4F[j].id){
                  this.fruta_kg.push(0);
                  break;
                }
                //Generar los array 
